...
// v4 TRAP 1 FIX: FieldValue.increment(1) instead of resultsSnap.get() per link
...
export async function saveResultToFirestore(
  ...
  // TIER 1: Sub-collection write — each link gets its own doc (zero contention)
  // scraping_tasks/{taskId}/results/{lid}
  await db
  ...
  // TIER 2: Master doc — atomic increment + completion check
  // v4 TRAP 1: FieldValue.increment(1) = ZERO extra reads (was: 1250 reads/task)
  // v5 TRAP 5: CONDITIONAL — deferred (pending) links NEVER increment counter
  try {
    ...
    // Read master doc to check completion (1 read, not per-link)
    ...
    // Final status update — sirf EK BAAR jab sab links done
